...
unsigned long NB_OPERATIONS = 150;  //100  Define the number of xorshift to generate a sequence
const int REARRANGEMENT     = 35;   //Optimal number of xorshift of the sequence for almost equiprobable frequency occurence 
const int NB_FREQUENCIES    = 1000; //Number of frequencies generate by the PN generator

std::bitset<SEQUENCE_SIZE> truncate;
std::bitset<BPS> symbol;
std::vector<std::string> reduce_sequence;
std::vector<std::string> stockage(1000);
std::vector<int> stockage_frequency(1000);
